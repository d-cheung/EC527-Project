...
void FastHessian::buildResponseMap()
{
	...
	float * d_responses;
	unsigned char * d_laplacian;

	cudaMalloc((void **) &(d_responses), (w * h)*sizeof(float));
	cudaMalloc((void **) &(d_laplacian), (w * h)*sizeof(unsigned char));

	  // Extract responses from the image
	for (unsigned int i = 0; i < responseMap->size(); ++i)
	{
		buildResponseLayer(*((*responseMap)[i]), d_responses, d_laplacian);
	}
...
__global__ void buildCudaResponse(int rl_width, int step, int b, int l, int w, float inverse_area, float ** img, int Height, int Width, float * responses, unsigned char * laplacian)
{

	int ac = threadIdx.x + blockDim.x*blockIdx.x;
	int ar = blockIdx.y;
	if (ac >= rl_width) return;
	int height = Height;
	int width = Width;
	...
	float Dxx = + d_BoxIntegral(img, r - l + 1,     c - b, 2 * l - 1,         w, height, width)
		    - d_BoxIntegral(img, r - l + 1, c - l / 2, 2 * l - 1,         l, height, width) * 3;
	float Dyy = + d_BoxIntegral(img,     r - b, c - l + 1,         w, 2 * l - 1, height, width)
		    - d_BoxIntegral(img, r - l / 2, c - l + 1,         l, 2 * l - 1, height, width) * 3;
	float Dxy = + d_BoxIntegral(img,     r - l,     c + 1,         l,         l, height, width)
		    + d_BoxIntegral(img,     r + 1,     c - l,         l,         l, height, width)
		    - d_BoxIntegral(img,     r - l,     c - l,         l,         l, height, width)
		    - d_BoxIntegral(img,     r + 1,     c + 1,         l,         l, height, width);
	...
	responses[ar*rl_width + ac] = (Dxx * Dyy - (float)0.81 * Dxy * Dxy);
	laplacian[ar*rl_width + ac] = (unsigned char)(Dxx + Dyy >= 0 ? 1 : 0);

}

__global__ void buildCudaResponse(int rl_width, int step, int b, int l, int w, float inverse_area, float * img, size_t Pitch, int Height, int Width, float * responses, unsigned char * laplacian)
{
	...
	float Dxx = + d_BoxIntegral(img, r - l + 1,     c - b, 2 * l - 1,         w, pitch, height, width)
		    - d_BoxIntegral(img, r - l + 1, c - l / 2, 2 * l - 1,         l, pitch, height, width) * 3;
	float Dyy = + d_BoxIntegral(img,     r - b, c - l + 1,         w, 2 * l - 1, pitch, height, width)
		    - d_BoxIntegral(img, r - l / 2, c - l + 1,         l, 2 * l - 1, pitch, height, width) * 3;
	float Dxy = + d_BoxIntegral(img,     r - l,     c + 1,         l,         l, pitch, height, width)
		    + d_BoxIntegral(img,     r + 1,     c - l,         l,         l, pitch, height, width)
		    - d_BoxIntegral(img,     r - l,     c - l,         l,         l, pitch, height, width)
		    - d_BoxIntegral(img,     r + 1,     c + 1,         l,         l, pitch, height, width);
...
/// </summary>
/// <param name="rl"></param>
void FastHessian::buildResponseLayer(ResponseLayer &rl, float * d_responses, unsigned char * d_laplacian)
{
	...
	BlocksPerGrid = dim3(((rl.width / ThreadsPerBlock) + 1), rl.height);

#ifdef FLOAT_SINGLE
	buildCudaResponse <<< BlocksPerGrid, ThreadsPerBlock >>> (rl.width, step, b, l, w, inverse_area, img->Matrix, img->Pitch, img->Height, img->Width, d_responses, d_laplacian);
#endif

#ifdef FLOAT_DOUBLE
	buildCudaResponse <<< BlocksPerGrid, ThreadsPerBlock >>> (rl.width, step, b, l, w, inverse_area, img->Matrix, img->Height, img->Width, d_responses, d_laplacian);
#endif

	cudaThreadSynchronize();

	cudaMemcpy(rl.responses, d_responses, (rl.width * rl.height)*sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(rl.laplacian, d_laplacian, (rl.width * rl.height)*sizeof(unsigned char), cudaMemcpyDeviceToHost);
}
