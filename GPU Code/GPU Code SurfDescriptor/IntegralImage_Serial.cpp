...
const float IntegralImage_Serial::cR = (float).2989;
const float IntegralImage_Serial::cG = (float).5870;
const float IntegralImage_Serial::cB = (float).1140;


float IntegralImage_Serial::getValue(int y, int x)
{
//	return this->Matrix[y*Height+x];
	return this->Matrix[y][x];
}

void IntegralImage_Serial::setValue(int y, int x, float value)
{ 
...
IntegralImage_Serial::IntegralImage_Serial()
{
	this->Width = 0;
	this->Height = 0;
	this->Matrix = NULL;
...
IntegralImage_Serial::IntegralImage_Serial(int width, int height)
{
	this->Width = width;
	this->Height = height;
...
IntegralImage_Serial * IntegralImage_Serial::FromImage(bitmap_image &image)
{
	IntegralImage_Serial * pic = new IntegralImage_Serial(image.width(), image.height());
...
float IntegralImage_Serial::BoxIntegral(int row, int col, int rows, int cols)
{
...
/// <returns></returns>
float IntegralImage_Serial::HaarX(int row, int column, int size)
{
...
/// <param name="size"></param>
/// <returns></returns>
float IntegralImage_Serial::HaarY(int row, int column, int size)
{
